"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.getCourseById = exports.getAllCourses = exports.createCourse = void 0;
var courses_1 = __importDefault(require("../../models/courses"));
var jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
var moment_1 = __importDefault(require("moment"));
var createCourse = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, name_1, video_url, description, status_1, adminId, token, decodedToken, decodedAdminId, newCourse, savedCourse, err_1;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 2, , 3]);
                _a = req.body, name_1 = _a.name, video_url = _a.video_url, description = _a.description, status_1 = _a.status;
                adminId = req.headers['x-admin-id'];
                token = req.headers.authorization;
                console.log({ token: token });
                // Check if admin ID and token are provided
                if (!adminId || !token) {
                    return [2 /*return*/, res.status(400).json({ message: 'Admin ID and token are required' })];
                }
                decodedToken = jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET);
                decodedAdminId = decodedToken.admin.id;
                // Check if the extracted admin ID matches the one from headers
                if (decodedAdminId !== adminId) {
                    return [2 /*return*/, res.status(401).json({ message: 'Unauthorized' })];
                }
                newCourse = new courses_1.default({
                    name: name_1,
                    video_url: video_url,
                    description: description,
                    admin: adminId,
                    status: status_1,
                    date_created: (0, moment_1.default)().format()
                });
                return [4 /*yield*/, newCourse.save()];
            case 1:
                savedCourse = _b.sent();
                // Send success response
                res.status(201).json(savedCourse);
                return [3 /*break*/, 3];
            case 2:
                err_1 = _b.sent();
                console.error(err_1.message);
                res.status(500).send('Server Error');
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); };
exports.createCourse = createCourse;
var getAllCourses = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var courses, err_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, courses_1.default.find()];
            case 1:
                courses = _a.sent();
                console.log({ courses: courses });
                // Send the courses as a response
                res.json(courses);
                return [3 /*break*/, 3];
            case 2:
                err_2 = _a.sent();
                console.error(err_2.message);
                res.status(500).json({ message: 'Server Error' });
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); };
exports.getAllCourses = getAllCourses;
var getCourseById = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, course, err_3;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                id = req.params.id;
                return [4 /*yield*/, courses_1.default.findById(id)];
            case 1:
                course = _a.sent();
                // Check if the course exists
                if (!course) {
                    return [2 /*return*/, res.status(404).json({ message: 'Course not found' })];
                }
                // Send the course as a response
                res.json(course);
                return [3 /*break*/, 3];
            case 2:
                err_3 = _a.sent();
                console.error(err_3.message);
                res.status(500).json({ message: 'Server Error' });
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); };
exports.getCourseById = getCourseById;
//# sourceMappingURL=create.js.map